"""
Bonsai version of Bpod_npy_threshold_calculator2.py â€” v2.
...
if len(ind) == 0:
    ...
    #   (a) the first epoch's thresholds (lo_first, hi_first)  â†’ speed_old
    #   (b) the current epoch's thresholds (lo_cur, hi_cur)    â†’ speed_new
    ...
    # the trial is predicted to time out â†’ hit=False, rt_pred capped at 10.
    # ========================================================
    ...
    for si in range(len(switches)):
        ...
        for t in ref_trials:
            ...
            if not rew[t] or np.isnan(ref_speed_old[t]) or ref_speed_old[t] <= 0:
                # Skip trials that weren't a hit under old thresholds â€” matches
            ...
            if np.any(spd_new >= 3.3 - 1e-9):
                n_saturated += 1
            if mean_new <= 0:
                # New thresholds zero out the trajectory entirely â†’ infinite
                ...
                continue
            # No cap on rt_pred â€” a value > 10 is informative ("avg speed is
